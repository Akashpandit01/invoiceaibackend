...
def extract_text(path, filename):
    """
    OCR is disabled for deployment (Render doesn't support Tesseract).
    Returning mock text so app runs without breaking logic.
    """
    return "Invoice Date: 12/05/2024 Amount: 100 Vendor: Demo"
...
async def upload(file: UploadFile = File(...), authorization: str = Header(None)):
    ...
    supabase.table("invoices").insert({
        **parsed,
        ...
        "file_url": f"/uploads/{file.filename}",  # fixed for production
        "user_email": user
    }).execute()
